...
const MealPlanSection = () => {
  ...
  const filteredMeals = selectedMeals.filter((meal) => {
    const budgetMatch =
      filters.budget.length === 0 || filters.budget.includes(meal.budget);
    const mealTypeMatch =
      filters.mealType.length === 0 || filters.mealType.includes(meal.mealType);
  ...
  const mealsByType = mealTypesList.map((type) => {
    return (
      filteredMeals.find((meal) => meal.type === type) || {
        type,
  ...
  //         dayMeals[mealIndex] = {
  //           ...dayMeals[mealIndex],
  //           selected: !dayMeals[mealIndex].selected,
  //         };
  ...
  const transformAIMealData = (aiData) => {
    ...
    aiData.forEach((dayData) => {
      ...
      const meals = dayData.meals.map((meal) => {
        ...
        const randomBudget =
          budgetList[Math.floor(Math.random() * budgetList.length)];
    ...
    <div className="p-4 md:p-6 font-instrument-sans">
      ...
      <div className="mb-8">
        <p className="text-gray-600">
          Meal plan generation helps you structure meals ahead, so you can plan
          groceries and eat efficiently.
        </p>
      </div>
      ...
      <div className="md:flex md:flex-wrap md:justify-between space-x-4">
        ...
        <div className="flex gap-5 py-3 md:my-2 relative">
          ...
          >
            <IoFilterOutline
              className={`w-6 h-6 ${
                filters.budget.length || filters.mealType.length
                  ? "text-pryGreen"
                  : ""
              }`}
            />
      ...
      <div className="flex space-x-4 mb-4 overflow-x-auto">
        {activeTab === "ThisWeek" ? (
          daysOfWeek.map((day) => (
            <button
              key={day}
              className={`px-4 py-2 border rounded-lg text-sm font-semibold transition whitespace-nowrap ${
                selectedDay === day
      ...
      {isLoading ? (
        ...
        mealsByType.some((meal) => meal.name !== "No meal selected") ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {mealsByType.map((meal, index) => {
            ...
            return (
              <div
                key={index}
                className={`h-48 overflow-hidden relative rounded-lg transition-all duration-300 ${
              ...
              >
                <div className="absolute inset-0 bg-black/30 text-white p-4 flex flex-col justify-between items-start">
                  ...
                  <p className="text-sm font-semibold max-w-[80%] line-clamp-2">
                    ...
                    <button
                      className={`mt-2 ${
                        isSelected
                          ...
                          : "bg-pryGreen text-white"
                      } text-sm px-4 py-2 cursor-pointer rounded-lg w-full`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  <p className="text-gray-800">
                    Calories: {modalMeal.calories || "N/A"} kcal
                  </p>
                  <p className="text-gray-800">
                    Carbs: {modalMeal.carbs || "N/A"}
                  </p>
                  <p className="text-gray-800">
                    Protein: {modalMeal.protein || "N/A"}
                  </p>
                  <p className="text-gray-800">
                    Fats: {modalMeal.fats || "N/A"}
                  </p>
                  {modalMeal.waterIntake && (
                    <p className="text-gray-800">
                      Water Intake: {modalMeal.waterIntake}
                    </p>
                  )}
                  ...
                  {modalMeal.magnesium && (
                    <p className="text-gray-800">
                      Magnesium: {modalMeal.magnesium}mg
                    </p>
                  )}
                  {modalMeal.potassium && (
                    <p className="text-gray-800">
                      Potassium: {modalMeal.potassium}mg
                    </p>
                  )}
                  {modalMeal.zinc && (
                  ...
                  {modalMeal.selenium && (
                    <p className="text-gray-800">
                      Selenium: {modalMeal.selenium}μg
                    </p>
                  )}
                </div>
              </div>
              ...
              <button
                className={`w-full flex gap-x-2 items-center justify-center py-3 rounded-md font-medium ${
                  modalMeal.selected
                    ? "bg-white text-green-600 border border-green-500"
                    : "bg-pryGreen cursor-pointer text-white"
                }`}
                onClick={async () => {
                  try {
                    ...
                    const res = await fetch(
                      "http://localhost:3005/meal/store-meals",
                      {
                        headers: {
                          "Content-Type": "application/json",
                        },
                        ...
                        body: JSON.stringify({
                          name: modalMeal.name,
                          calories: modalMeal.calories,
                          nutrients: {
                            potassium: modalMeal.potassium,
                            zinc: modalMeal.zinc,
                            calcium: modalMeal.calcium,
                            iron: modalMeal.iron,
                            fats: modalMeal.fats,
                            protein: modalMeal.protein,
                            selenium: modalMeal.selenium,
                            waterIntake: modalMeal.waterIntake,
                          },
                        }),
                      }
                    );
